...
- `spawn_test_slot_session(slot_name, prompt, mode, ...)` — create a fresh SDK chat session through a Glimmung test slot's own Tank orchestrator, then queue the first prompt. Use this for test-slot validation; it requires a slot name such as `tank-operator-slot-2` and refuses production-ish targets.
